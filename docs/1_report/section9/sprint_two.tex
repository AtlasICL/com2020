...
\begin{itemize}
    ...
    \item Each symptoms should then be associated with a common problem.
    \item Each problem should then have a clear series of troubleshooting steps to fix it.
    \item Lastly each problem should have clear verification criteria to check it has been solved.
\end{itemize}
...
\begin{itemize}
  ...
  \item Fr each criteria that's met, the system must suggestion a concrete change to a design parameter that will improve balance.
...
\begin{itemize}
  ...
  \item 25\% of these decisions must be flawed, and 75\% must be good.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item The player must always be able to use the default attack ability at will while in an encounter. 
  \item The player must be able to use any mundane attack abilities they've purchased in the shop at will in an encounter
...
\begin{itemize} 
  ...
  \item While in an encounter, if the player has sufficient magic they must be able to use the magic ability if they've purchased it.
...
\textbf{Acceptance Criteria} 
\begin{itemize} 
  \item Enemies must be able to use both magic and mundane attack abilities at will against the player.
...
\begin{itemize} 
  ...
  \item These abilities must affect the player in an encounter by doubling the damage they deal when the damage being dealt by them is that the upgrade provides resistance to.
  \item These must not be purchasable if the player does not have enough coins.
  \item These must reduce the amount of coins the player has by their cost when purchased.
  \item They must only be purchased once per run. 
\end{itemize}
...
\begin{itemize} 
  ...
  \item They must then be able to use one of their attack abilities.
  \item Then all enemies must use their attack abilities.
...
\begin{itemize} 
  ...
  \item The encounters on each stage should progressively get harder.
\end{itemize}
