...
This section follows the exact same format as the sprint one prioritised requirements. Each story's value is calculated assuming the work done on that story is what was planned in the previous sprint (So anything in the Must, Should, or Could category is done). Due to the adaptability of scrum, it is expected that both the project backlog and sprint two plan will have changed by the end of sprint two, to ensure the plan is both realistic and adaptive. 
...
\begin{itemize} 
  ...
  \item Each story must have a priority associated with it for that sprint. The priority must either be Must, Should, Could, or Won't. 
...
\begin{itemize} 
  ...
  \item Classes that share an identical structure with minor variation need only their category to be explained.
  \item All structural design choices must be justified and explained.
  \item The limitations of all specified design choices must be stated, and why they are acceptable must be explained.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize} 
  \item Each component that accesses data must be explained, describing what data it accesses, what it does with it, and why it needs to access this data.
  \item Each data store must be explained, stating what data it stores and in what format. 
\end{itemize}
...
\begin{itemize} 
  ...
  \item The evaluation must contain a discussion on the fairness of the system as a whole.
  \item The evaluation must contain a discussion on any unintended side effects of balancing decisions and game design.
\end{itemize}
...
\begin{itemize}
  ...
  \item Must discuss what areas the system can improve upon in the future.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item The guide must contain clear instructions for how to set up and run the Java application.
  \item There may be instructions for different systems, but there must be a way to run the application on any desktop system that runs Windows, MacOS or Linux.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item The guide must contain clear instructions for how to set up and run the Python application.
  \item There may be instructions for different systems, but there must be a way to run the application on any desktop system that runs Windows, MacOS or Linux.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item There must be clear instructions on how to run the test suite on the source code.
  \item The version of the testing framework must be provided.
  \item The running instructions must allow the running of the test on any desktop system that runs either Windows, MacOS or Linux.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item There must be 15 automated tests, consisting of both unit and integration tests.
  \item For each automated test it must be outlined what component is being tested.
  \item For each automated test it must be outlined what the expected output is.
  \item For each automated test the actual output of the system must be provided, and thus evidence that the system passes the test.
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item There must be 8 end to end tests.
  \item Evidence that the tests are successful must be given.
  \item The happy path and failure cases for each test must be described.
  \item The expected results of the tests must be described.
\end{itemize}
...
\begin{itemize}
  ...
  \item Must speak about similar solutions.
...
\begin{itemize}
  ...
  \item Must demonstrate a run of the game up to and including stage 3.
  ...
  \item Must demonstrate the viewing of the telemetry disclosure. 
  \item Must show the handover pack.
  \item Must show how a maintainer would add a new stage. 
  \item Must be rehearsed before the live demo, to ensure it works correctly.
  \item Must have a backup demonstration, must it go wrong.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item Must outline the areas in which the system is weakest.
  \item Must briefly outline what could be done in the future to address these weaknesses.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item Must outline the ethical considerations of the system.
  \item Must outline what telemetry is collected and for what purpose.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item Must show a snapshot of the scrum board.
  \item Must contain relevant information to the current sprint. 
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
    \item Must contain clear instructions on how to launch both the python and java application, once all prerequisites are configured.
    \item Must contain clear instructions on how to use the Java application, including an explanation of each user menu.
    \item Must contain clear instructions on how to use the Python application, including an explanation of each user menu.
    \item Must contain instructions on how to manage user data and roles.
    \item Must contain instructions on how to manage the telemetry data.
    \item Must contain instructions on how to interpret any logging systems, including the decision log. 
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
    \item Must contain a list of many common symptoms of problems.
    \item Each symptoms must then be associated with a common problem.
    \item Each problem must then have a clear series of troubleshooting steps to fix it.
    \item Lastly each problem must have clear verification criteria to check it has been solved.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
    \item Must contain clear instructions on how to add additional telemetry events.
    \item Must contain clear instructions on how to add additional abilities (both passive and active).
    \item Must contain clear instructions on how to add additional damage types.
    \item Must contain clear instructions on how to add additional enemies (both normal and boss)
    \item Must contain clear instructions on how to add additional encounters.
    \item Must contain clear instructions on how to add additional stages.
    \item Must contain an overview of the system architecture, explaining it without justifying the design.
    \item Must contain an explanation of how both applications manage security, including authentication and access control.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
    \item Must contain a clear explanation of each data store and it's purpose.
    \item Must contain a description of what kind of data is stored in each data store.
    \item Must explain which processes interact with each data store, and in which way they interact with it. 
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
    \item Must contain an overview of the format each data store uses to store data.
    \item For each data store, must provide a clear schema for how data is stored.
    \item Must specify clearly the domain of all fields in all data stores.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
    \item An authenticated user must be able to view what their role is.
    \item An authenticated user must be able to view what their username is.
    \item An authenticated user must be able to log out from this menu.
\end{itemize}
...
\begin{itemize}
  ...
  \item This must then be displayed to the user. 
  \item It must be able to be switched from viewing actual user data, to simulation generated telemetry data.
  \item The system must highlight significant spikes to the designer.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item The system must be able to determine if a player run was fast or slow.
  \item The system must be able to determine if a player run was aggressive or defensive.
  \item The system must allow the comparison of different speeds on key metrics including lives, coins and health as runs progress.
  \item The system must allow the comparison of different aggressiveness values on key metrics including lives, coins and health as runs progress.
...
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item The system must calculate how many coins user's character's accumulated on each stage along with how much health they finish each stage with, and how many lives they have, for each difficulty.
  \item This must then be displayed to the user. 
  \item It must be able to be switched from viewing actual user data, to simulation generated telemetry data.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item The system must read the telemetry data and evaluate it against 6 suggestion criteria.
  \item For each criteria that's met, the system must suggestion a concrete change to a design parameter that will improve balance.
  \item This suggestions must be displayed to the user.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item Designers must be able to view a log of changes to design parameters.
  \item Each change must have an associated explanation as to why.
  \item Those explanations must be able to link to telemetry data evidence.
\end{itemize}
...
\begin{itemize}
    ...
    \item The application must provide clear errors to the user when failing to validate them.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item There must be an example dataset of 1,500 seeded telemetry events.
  \item The dataset of seeded telemetry must be have been generated by sources across all stages.
  \item The dataset of seeded telemetry must be have been generated by sources across all difficulties.
  \item The dataset of seeded telemetry must be have been generated by sources across 40 or more users.
  \item The dataset of seeded telemetry must be have been generated by sources across 80 or more sessions.
  \item The dataset of seeded telemetry must be contain 150 anomalous values, that do not follow the specification.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item There must be an example decision log containing 30 decisions.
  \item 10 of these decisions must be flawed, and 20 must be good.
\end{itemize}
...
\begin{itemize}
    ...
    \item A user with the developer role must be able to assign roles to other users.
\end{itemize}
...
\begin{itemize}
    ...
    \item While disabled telemetry events must not be recorded.
...
\begin{itemize}
    ...
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
    \item A designer in settings must be able to view the enemy health multiplier for each difficulty.
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
    \item A designer in settings must be able to view the player maximum health for each difficulty.
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
    \item A designer in settings must be able to view the upgrade price multiplier for each difficulty.
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
    \item A designer in settings must be able to view the enemy damage multiplier for each difficulty.
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
    \item A designer in settings must be able to view the player maximum magic for each difficulty.
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
    \item A designer in settings must be able to view the magic regeneration rate for each difficulty.
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
    \item A designer in settings must be able to view the number of items available in the shop for each difficulty.
    \item A designer in settings must be able to set its value for each difficulty.
    \item The value of the design parameter must persist beyond the lifetime of the game.
\end{itemize}
...
\begin{itemize}
  ...
  \item The run must continue until the agent beats the game or dies.
\end{itemize}
...
\begin{itemize}
  ...
  \item There must always be 1 developer account.
\end{itemize}
...
\begin{itemize} 
  ...
  \item If a player dies while having 1 or more lives they must restart the same encounter. Their health and magic must be reset to their starting values, as must the health of all enemies within the encounter. 
...
\begin{itemize} 
  ...
  \item On stages 3, 6, 9 and 10 the player must encounter a boss enemy as described in the game design documentation.
  \item There must be a separate collection of encounters for stages 1 and 2, as there is for 4 and 5, and 7 and 8.
  \item The encounters on each stage must progressively get harder.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize} 
  \item If a player completes the encounter on stage 10 they must complete the run.
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item The game must feature the boss encounter start event.
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player start a boss encounter.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item The game must feature the boss encounter fail event.
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player dies in a boss encounter.
\end{itemize}
...
\begin{itemize}
  \item The game must feature the boss encounter fail event.
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player completes a boss encounter by killing the boss
\end{itemize}
...
\begin{itemize}
  ...
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created when a game run ends, either by the user completing the run by completing the last encounter, or by their character dying.
\end{itemize}
