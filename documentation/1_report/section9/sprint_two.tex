...
This section follows the exact same format as the sprint 1 prioritised requirements. Each story's value is calculated assuming no prior work has been done on the story, even if it was in the must or should criteria of sprint one's plan. Due to the adaptability of scrum, it is expected that both the project backlog and sprint two plan will have changed by the end of sprint two, to ensure the plan is both realistic and adaptive. 
...
\begin{itemize} 
  ...
  \item A thoughtful interpretation of each measure and metric should be provided.
  \item A critical reflection of any trade-offs made in the system should be included, including the impact of those trade-offs. 
  \item There must be a discussion on the fairness of the game.
  \item There must be a discussion on the unintended side consequences of the game design. 
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item There must be 15 or more automated tests.
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item There must be eight or more end to end tests.
  \item Evidence that each test is successful should be given.
  \item The happy path and failure cases for each test must be described.
  \item The expected results of each test should be described.
\end{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item Should highlight the core strengths of the system.
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item Should take around 6 minutes to complete.
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item Should outline the areas in which the system is weakest.
  \item Should outline in what areas the system can be improved, and how this could be done.
\end{itemize}
\subsubsection{Maintenance and Troubleshooting Guide}
\paragraph{Maintainance Instructions}
\textbf{Value:} 5\quad \textbf{Priority:} Must\\
\textbf{Acceptance Criteria} 
\begin{itemize}
...
\textbf{Acceptance Criteria}
\begin{itemize}
  \item The system should contain a tab for displaying changes made to the design parameters.
  \item Each change must be displayed to the user, along with the explanation the designer gave for the change to that parameter.
\end{itemize}
...
\begin{itemize}
  ...
  \item A designer must be able to view the decision log. 
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item The system should read the telemetry data and evaluate it against six or more suggestion criteria.
  \item For each criteria that is met, the system should suggest a concrete change to a design parameter to improve the balance of the game. 
\end{itemize}
...
\begin{itemize}
  ...
  \item The system must be able to detect telemetry events from the future and ignore them.
  \item The system must be able to detect telemetry events that arrive in an impossible order and ignore them.
\end{itemize}
...
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item The dataset of seeded telemetry must be have been generated by sources across 80 or more users.
\end{itemize}
...
\begin{itemize}
  ...
  \item When changing a design parameter, it must prompt the user for an explanation as to why. 
...
\begin{itemize}
  ...
  \item Any change to a must be accompanied by an explanation as to why.
  \item Both the change and explanation should be stored in the decision log. 
\end{itemize}
...
\begin{itemize}
  ...
  \item Stages one and two must draw their encounters from the same unique pool.
  \item Stage three must contain a unique boss.
  \item Stages four and five must draw their encounters from the same unique pool.
  \item Stage six must contain a unique boss.
  \item Stages seven and eight must draw their encounters from the same unique pool.
  \item Stage nine must contain a unique boss.
  \item Stage ten must contain the unique final boss.
  \item The run must end after stage ten is complete. 
\end{itemize}
...
\begin{itemize}
  ...
  \item There must be a passive ability for each damage type, that allows the user's character to deal double that damage type, in accordance with the game design documentation. 
\end{itemize}
...
\begin{itemize}
  ...
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player enters an encounter with a boss.
\end{itemize}
...
\begin{itemize}
  ...
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player dies an encounter with a boss.
\end{itemize}
\paragraph{Boss Encounter Complete}
\textbf{Value:} 1 \quad \textbf{Priority:} Must\\
\textbf{Acceptance Criteria} 
\begin{itemize}
  \item The game must feature the boss encounter complete event.
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player completes an encounter with a boss by killing it.
\end{itemize}
\paragraph{Gain Coin}
\textbf{Value:} 1 \quad \textbf{Priority:} Must\\
\textbf{Acceptance Criteria} 
\begin{itemize}
  ...
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player gains coins after an encounter.
\end{itemize}
...
\begin{itemize}
  ...
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player buys an upgrade in the shop.
\end{itemize}
...
\begin{itemize}
  ...
  \item The event must have the fields specified in the telemetry schema.
  \item The event must be created each time the player kills an enemy in an encounter, including bosses.
\end{itemize}