...
\\ 
Given a user doesn't have an account, they can create an account by entering a username and password. Given a user does have an account they can sign in using their username and password. Given a user is signed in they can chose to start a game, view and edit their settings, or sign out. 
...
\textbf{Acceptance Criteria} 
\\ 
Given a user has been authenticated with the player role, they can play the game and toggle their telemetry enabled setting. Given a user has been authenticated with the designer role, they can do anything a player can, as well as change the design parameter values in settings. Given a user has been authenticated with the developer role, they can do anything a designer can do, and assign roles to other users in settings. 
\subsubsection{User Settings (Must)}
The game must contain a settings screen the following user Settings.
\begin{description}
    \item[\textbf{Telemetry Enabled}] - whether the game records telemetry events or not. On by default. The menu screen should display a warning notifying users when telemetry is enabled.
\end{description}
...
\\
Given a user is authenticated and in the main menu, they can access the settings screen. While in the main menu they are notified if they have telemetry enabled. Given a user is in settings they can toggle whether they send telemetry events or not, this is by default on. Given the user is a designer they can edit the design parameter values and save those changes. Given the user is a developer they can assign roles to any other user. 
